...
of $df_h >= 2$ degrees of freedom can be partitioned into $df_h >= 2$ separate 1 df
...
The `heplot()` `pairs.mlm() and `heplot3d()` functions provide 2D and 3D views of all effects in a MLM in **variable** space.
...
Plot canonical discriminant scores in 2D space, with the data ellipses for each group:
...
Show this as an HE plot in canonical discriminant space:
